...
const mockApi = (() => {
  ...
  function validate(payload) {
    ...
    const occupant_name = (payload.occupant_name || "").trim();

if (occupant_name.length > 0 && occupant_name.length < 3) {
    errors.occupant = "Occupant name must contain at least 3 letters.";
}

if (occupant_name && !/^[A-Za-z ]+$/.test(occupant_name)) {
    errors.occupant = "Only alphabets are allowed.";
}
  ...
  return {
    ...
    update(id, payload) {
        if (savingRecords.has(id)) {
         return delay(new Error("Another user is already updating this record."));
       }
      savingRecords.add(id);
      const errors = validate(payload);
      if (Object.keys(errors).length) {
        const err = new Error("Validation failed");
        err.fieldErrors = errors;
        savingRecords.delete(id);
        return delay(err);
      }
      try {
        const records = readStore();
...
function clearForm() {
  ...
  resolvedDate.value = "";
  ["roomNo","block","occupant","reportedDate","resolvedDate"].forEach(clearFieldError);
}
...
Promise.allSettled([
    mockApi.update(1, samplePayload),
    mockApi.update(1, samplePayload)
]).then(results => {
    console.log("Concurrent Update Test:", results);
});